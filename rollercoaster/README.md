**NOTE:** The README below is the original readme of the Rollercoaster
simulation. The steps to run the simulation (using the Docker image and the
Jupyter notebooks) still works. As such, the README has been preserved. The
code however contains the PolySphinx adjustments.

In addition to the PolySphinx changes, we did add a script
`docker_04a_run_missing.sh` which only runs the scenarios that are still
missing. This allows you to resume a previously interrupted run.
