...
from simulation.messages import TAG_PAYLOAD, TAG_DROP, TAG_LOOP, create_wrapped_message, WrappedMessage, Message, wrap_messages_in_multi_message, ApplicationMessage
...
class LoopixConfiguration():
    ...
    def __init__(
        ...
        self.round_multiplicator = round_multiplicator
...
class User(SimulationObject):
    ...
    def __init__(self, name, provider, mix_network, config, online_schedule=None):
        ...
        self.round_multiplicator = config.round_multiplicator
    ...
    def set_split(self, split):
        ...
        # To stay constant in bandwidth, we may not increase the sending rate!
        # This will lead to fewer messages being sent, but each message will be
        # larger. This is in contrast to the original Rollercoaster simulation,
        # which keeps a constant "sending frequency" on the outside.
    ...
    def tick(self, sim):
        ...
        if sim.rnd.poisson_event(self.rate_payload):
            for _ in range(self.round_multiplicator):
    ...
    def _send_waiting_split_messages(self, sim):
        ...
        if all(isinstance(m, WrappedMessage) for m in messages):
            assert len(messages) == 1
            multi_message = messages[0]
        else:
