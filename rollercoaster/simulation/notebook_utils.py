...
from simulation.multicast import create_factory, Group, SequentialUnicastStrategy, RollercoasterStrategy, PolysphinxStrategy
...
def create_strategy_factory(name):
    ...
    if parts[0] == 'unicast':
        ...
        return create_factory(SequentialUnicastStrategy, **kwargs)
        ...
        for arg in parts[1:]:
            if arg.startswith('p'):
                ...
                kwargs['max_level'] = int(arg[1:])
            else:
                raise ValueError("Unknown parameter: {}".format(arg))

        return create_factory(PolysphinxStrategy, **kwargs)
